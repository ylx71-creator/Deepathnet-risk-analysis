"""
Run early-concatenation baselines on an independent test set for drug response
prediction.

Usage example:
    python scripts/baseline_independent_test.py \
...
if data_type[0] != "DR":
    # Keep only modalities requested in the config (or tissue features).
    data_input_train = data_input_train[
        [
            x
            for x in data_input_train.columns
...
for i in trange(num_targets):
    # Each column after the target block is treated as a feature set for one target.
    ...
    logger.info(
        f"Running {train_df.columns[i]} Train:{X_train.shape} Test:{X_test.shape}"
    )
    ...
    if configs["task"].lower() == "classification":
        ...
        score_dict = {"target": train_df.columns[i], "auc": val_auc, "acc": val_acc}
    else:
